...
def Punkte(Standort, Abfrage, TSstartDate, TSendDate):
    ...
    try:
        ...
        ).properties(width= 450, height= 400, title=f"Vergleich {Abfrage_typ} im Zeitraum vom {pd.to_datetime(TSstartDate).strftime('%m.%d.%Y')} bis {pd.to_datetime(TSendDate).strftime('%m.%d.%Y')} im Raum Zürich")
